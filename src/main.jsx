...
const starterChats = [
  { id: 1, title: 'Welcome to Forge AI', preview: 'A private space for your thoughts', time: 'Now', active: true },
...
function normalizeChats(value) {
  if (!Array.isArray(value)) return starterChats
  return value.map((chat) => chat?.title === 'Welcome to Local / AI' ? { ...chat, title: 'Welcome to Forge AI' } : chat)
}
...
function App() {
  const [chats, setChats] = useState(() => normalizeChats(readStoredJson('local-ai-chats', starterChats)))
  ...
  useEffect(() => {
    ...
    const state = { chats, selectedChat, messagesByChat, endpoint, mode, webSearchEnabled, graftRoot, graftEnabled, locationHint, fileToolsRoot, fileToolsEnabled, memory, theme }
    ...
    saveResult?.catch(() => {})
  }, [storageReady, chats, selectedChat, messagesByChat, endpoint, mode, webSearchEnabled, graftRoot, graftEnabled, locationHint, fileToolsRoot, fileToolsEnabled, memory, theme])
  useEffect(() => {
    const refresh = async () => {
  ...
  useEffect(() => {
    ...
    window.localAI.onUpdateStatus((status) => {
      if (status.type === 'checking') setGithubUpdateStatus('Checking GitHub for updates…')
       if (status.type === 'available') { const next = { available: true, version: status.version, releaseName: status.releaseName, downloaded: false, source: 'github', installOnQuit: true }; setGithubUpdateInfo(next); setUpdateInfo(next); setGithubUpdateStatus(`Version ${status.version} is downloading in the background`); setUpdateStatus(`Version ${status.version} is downloading in the background`) }
      if (status.type === 'current') { const next = { available: false, version: status.version, downloaded: false, source: 'github' }; setGithubUpdateInfo(next); setUpdateInfo(next); setGithubUpdateStatus(`Up to date (${status.version})`); setUpdateStatus(`Up to date (${status.version})`) }
       if (status.type === 'progress') { setGithubUpdateStatus(`Downloading update in the background… ${Math.round(status.percent || 0)}%`); setUpdateStatus(`Downloading update in the background… ${Math.round(status.percent || 0)}%`) }
       if (status.type === 'downloaded') { setGithubUpdateInfo((current) => ({ ...(current || {}), available: true, version: status.version, downloaded: true, source: 'github', installOnQuit: true })); setUpdateInfo((current) => ({ ...(current || {}), available: true, version: status.version, downloaded: true, source: 'github', installOnQuit: true })); setGithubUpdateStatus(`Version ${status.version} is ready; it will install when Forge AI closes`); setUpdateStatus(`Version ${status.version} is ready; it will install when Forge AI closes`) }
  ...
  async function checkForAppUpdate() {
    if (!window.localAI?.checkGitHubUpdate) {
      setUpdateStatus('GitHub updates are available in the installed Forge AI application.')
      return
    }
  ...
  async function checkGitHubForUpdate() {
    ...
    try {
      ...
      const next = result ? { ...result, source: 'github', downloaded: false, installOnQuit: true } : null
  ...
  async function exportExchange(assistantIndex) {
    ...
    const title = chats.find((chat) => chat.id === selectedChat)?.title || 'Forge AI exchange'
  ...
  return (
    <div className={`app-shell theme-${theme} ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
      ...
      <aside className="sidebar ollama-sidebar">
        ...
        <nav className="ollama-nav" aria-label="Forge AI workspace">
         ...
         <div className="sidebar-copyright" aria-label="Copyright">© 2026 TinoGoo · Original work, locally crafted.</div>
       </aside>
